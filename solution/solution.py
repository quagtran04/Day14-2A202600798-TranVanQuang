...
class RAGASEvaluator:
    ...
    def evaluate_conciseness(self, answer: str, max_words: int = 80) -> float:
        """
        Custom bonus metric: reward answers that stay concise.

        Returns 1.0 when the answer is at or below max_words. Longer answers
        are penalized gradually, reaching 0.0 at 2 * max_words or above.
        This metric is intentionally separate from overall_score().
        """
        if max_words <= 0:
        ...
        return _clamp_score(1.0 - overflow / max_words)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "Return JSON only, mapping criterion names to numeric scores.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{criteria}"
        )
        ...
        try:
            ...
            scores = {
                criterion: _clamp_score(float(parsed[criterion]))
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            for key, value in scores.items():
                ...
                if key_lower.startswith(("first", "a_", "answer_a", "response_a")):
                    first_scores.append(numeric)
                elif key_lower.startswith(("second", "b_", "answer_b", "response_b")):
                    other_scores.append(numeric)
        ...
        if first_scores and other_scores:
            positional_bias = (
                ...
                > sum(other_scores) / len(other_scores) + 0.1
            )
        return {
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if len(sorted_scores) >= 2 and sorted_scores[1] - sorted_scores[0] < 0.05:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = suggestions[index - 1] if index - 1 < len(suggestions) else "Review and triage"
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0 or categories.get("off_topic", 0) > 0:
            suggestions.append("Add intent checks and few-shot examples so answers stay aligned with the question")
        if categories.get("incomplete", 0) > 0:
            suggestions.append("Increase retrieved context coverage and ask the generator to answer every required point")

        if len(suggestions) < 3:
            low_faith = sum(1 for failure in failures if failure.faithfulness < 0.5)
            low_rel = sum(1 for failure in failures if failure.relevance < 0.5)
            low_comp = sum(1 for failure in failures if failure.completeness < 0.5)
            ranked = sorted(
                [
                    (low_faith, "Tune retriever top-k, chunking, and reranking to improve evidence quality"),
                    (low_rel, "Rewrite the system prompt to prioritize direct answers before extra explanation"),
                    (low_comp, "Add completeness checks against expected answer facets before final response"),
                ],
                reverse=True,
            )
